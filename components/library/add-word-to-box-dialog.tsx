...
export function AddWordToBoxDialog({ boxId, open, onOpenChange, existingWordIds }: AddWordToBoxDialogProps) {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>Search your vocabulary database and add words instantly.</DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                    ...
                    <div className="max-h-[320px] overflow-y-auto border rounded-lg">
                            ...
                            <Table className="text-sm">
                                ...
                                <TableBody>
                                    {searchResults.results.map((word: Doc<"words">) => {
                                        const alreadyInBox = existingWordIds.has(word._id) || recentlyAddedIds.has(word._id);
                                        return (
                                            <TableRow key={word._id}>
                                                ...
                                                <TableCell className="text-right">
                                                    ...
                                                    >
                                                        {alreadyInBox ? "In collection" : pendingWordId === word._id ? "Adding" : "Add"}
                                                    </Button>
                                                </TableCell>
                                            </TableRow>
                                        );
                        ...
                        {searchResults?.results && searchResults.results.length === 0 && searchTerm.trim().length > 0 && (
                            <div className="px-6 py-10 text-center text-sm text-muted-foreground">
