...
import { mutation, query, internalMutation, MutationCtx, DatabaseReader } from "./_generated/server";
...
const throwIfUnauthenticated = async (ctx: { auth: Auth }) => {
...
const getCurrentUser = async (ctx: { auth: Auth, db: DatabaseReader }) => {
...
export const getUserWordBoxes = query({
    args: {},
    handler: async (ctx, args) => {
...
export const getWordBoxWords = query({
    args: { 
